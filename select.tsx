...
type SelectContextValue = {
  ...
  contentId: string;
  value?: SelectType extends 'multiple' ? string[] : string;
  onValueChange(value: SelectType extends 'multiple' ? string[] : string): void;
  open: boolean;
  required?: boolean;
...
interface SelectProps {
  ...
  value?: SelectType extends 'multiple' ? string[] : string;
  defaultValue?: SelectType extends 'multiple' ? string[] : string;
  onValueChange?(
    value: SelectType extends 'multiple' ? string[] : string,
  ): void;
  open?: boolean;
    ...
    <PopperPrimitive.Anchor asChild {...popperScope}>
      <Primitive.button
        ...
        onKeyDown={composeEventHandlers(triggerProps.onKeyDown, (event) => {
          // Если тип селекта - 'multiple' и поле в фокусе, не будет происходить автоподстановки подходящего значения по нажатию на клавиатуру
          if (context.selectType === 'multiple') return;
  ...
  (props: ScopedProps<SelectValueProps>, forwardedRef) => {
    ...
    return (
      <Primitive.span
        ...
        style={{
          pointerEvents: context.selectType === 'multiple' ? undefined : 'none',
        }}
      >
        ...
        if (context.selectType === 'multiple') {
          isSelected
            ? context.onValueChange(
                // @ts-expect-error Не выводит массив строк в тип
...
>((props: ScopedProps<SelectItemIndicatorProps>, forwardedRef) => {
  ...
  return context.selectType === 'multiple' && itemContext.isSelected ? (
...
const SCROLL_BUTTON_IMPL_NAME = 'SelectScrollButtonImpl';
...
SelectScrollButtonImpl.displayName = SCROLL_BUTTON_IMPL_NAME;
